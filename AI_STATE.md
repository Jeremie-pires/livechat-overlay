...
- **Système de présence (Rooms)** : `ClientSession` ajouté au schéma Prisma (`token @id @default(uuid())`, `discordUserId`, `displayName`, `guildId`, `@@unique([discordUserId, guildId])`). `src/services/presenceStore.ts` (nouveau) : Map en RAM `guildId → socketId → {displayName, connectedAt}`. `socketLoader.ts` : `join-room` accepte `string | { id, token }` — si token valide, ajoute à presenceStore et broadcast `presence:update` à la room ; `disconnecting` retire le socket et re-broadcast. `clientCommand.ts` : génère ou retrouve le token persistant via `findFirst` + `create`, l'affiche dans l'embed (éphémère). `client.html` : passe `{ id, token }` dans `join-room` si token présent dans l'URL. `statsRoutes.ts` : ajoute `presence: presenceStore.getAll()` à la réponse. `dashboardRoutes.ts` : route `GET /api/presence/:guildId` (auth par session OU clientToken), CSS `.server-presence`, `renderServers(guilds, presence)` affiche un badge de présence par serveur. Desktop Electron : `clientToken` ajouté à `AppSettings`, `getOverlayUrl()` appende `&token=...`, IPC `app:get-presence` fait un fetch sécurisé, champ Token dans l'UI, summary grid 2×2 avec cellule "Connectés", polling 15 s via `startPresencePolling`/`stopPresencePolling`.
